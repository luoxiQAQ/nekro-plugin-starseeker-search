...
插件注册了以下聊天指令，可在聊天窗口直接使用：

| 指令 | 别名 | 说明 |
| --- | --- | --- |
| `/soutu` | `/搜图`、`/reverse_image` | 自动提取最近一条图片消息并执行以图搜图 |

用法：

1. 先在聊天中发送（或由别人发送）一张图片。
2. 发送 `/搜图` 即可，插件会自动查找最近 20 条消息中的第一张图片进行搜索。
3. 可选参数 `max_results`：`/搜图 5` 返回最多 5 条结果。
